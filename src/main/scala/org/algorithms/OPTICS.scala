...
class OPTICSPoint(_pointIndex: Int, _reachDistance: Float = -1f, _coreDistance: Float = -1f,
                  _dataPoint: Array[Float] = null, _isProcessed: Boolean = false,
                  _clusterId: Int = -1) {
...
object OPTICS {
  ...
  def apply(datasetPoints: Array[OPTICSPoint], eps: Float, minPts: Int) :
                    ListBuffer[OPTICSPoint] = {
      ...
      if(point.isProcessed == false) {
        ...
        if (point.coreDistance != -1f) {
          ...
          while(!priorityQueue.seeds.isEmpty) {
            ...
            pointNeighbor.coreDistance = coreDistance(datasetPoints, neighborhoodPoints,
              pointNeighbor, eps, minPts)
            resulList += pointNeighbor

            if(pointNeighbor.coreDistance != -1f) {
              update(datasetPoints, neighborhoodPoints, pointNeighbor, priorityQueue, eps, minPts)
            }
          }
        }
      }
  ...
  def update(datasetPoints: Array[OPTICSPoint], neighborhoodPoints: Set[OPTICSPoint], point:
                    OPTICSPoint, priorityQueue: OPTICSPriorityQueue, eps: Float, minPoints: Int)
                    : Unit = {
    ...
    for (neighborhoodIterator <- neighborhoodPoints) {
      if(neighborhoodIterator.isProcessed == false) {
        var newReachDistance = math.max(coreDistance,
          Similarity.euclidean(neighborhoodIterator.dataPoint, point.dataPoint))
  ...
  def coreDistance(datasetPoints: Array[OPTICSPoint], neighborhoodPoints: Set[OPTICSPoint],
                   point: OPTICSPoint, eps: Float, minPoints: Int) : Float = {
  ...
  def regionQuery(datasetPoints: Array[OPTICSPoint], point: OPTICSPoint, eps: Float) :
                    Set[OPTICSPoint] = {
